...
public class RateLimitedLogger implements Logger {
     ...
     */
    public static RateLimitedLoggerBuilder.MissingRateAndPeriod wrap(Logger logger) {
        return new RateLimitedLoggerBuilder.MissingRateAndPeriod(Preconditions.checkNotNull(logger));
    }

    /**
     * Same as {@link #wrap(Logger)} but gets the Logger from the {@link LoggerFactory} using the given class.
     */
    public static RateLimitedLoggerBuilder.MissingRateAndPeriod create(Class<?> clazz) {
        return new RateLimitedLoggerBuilder.MissingRateAndPeriod(
                Preconditions.checkNotNull(LoggerFactory.getLogger(clazz)));
    }

    // package-local ctor called by the Builder
    RateLimitedLogger(Logger logger, RateLimitedLogWithPattern.RateAndPeriod rateAndPeriod, Stopwatch stopwatch, Optional<CounterMetric> stats, Registry registry) {
        this.logger = logger;
